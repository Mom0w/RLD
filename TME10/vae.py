...
SAVE_DIR_CHECKPOINT = Path("checkpoints")
SAVE_DIR_IMAGES = Path("results")
...
def generate(vae_model, inp, from_latent=True, path=None):
    """
    inp : en entrée soit un batch de variable latentes si from_latent=True
        ou alors un batch d'images à reconstruire
    """
    if from_latent:
    ...
    if from_latent:
        ...
        plt.axis("off")
        plt.title(f"Images reconstruites")
...
def train(vae_model, optimizer, loaders, writer, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        print(f"Epoch {epoch+1}/{args.num_epochs} - (Durée: {duration :.2} min): \
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Entraînement d'un VAE")
    parser.add_argument('--batch_size', default=100, type=int, help='Batch size')
    parser.add_argument('--latent_dim', default=3, type=int, help="Dimension de l'espace latent")
    parser.add_argument('--architecture', default="conv", choices=["linear", "conv"],
                        type=str, help="Type d'architecture: linear ou conv")
    parser.add_argument('--lr', default=0.001, type=float, help='Learning Rate')
    parser.add_argument('--num_epochs', default=15, type=int, help="Nombre d'epochs")
    ...
    for architecture in ["conv", "linear"]:
        for latent_dim in [2, 16, 64]:
            args.architecture = architecture
            args.latent_dim = latent_dim

            outdir = Path(f'runs/{args.architecture}_{args.latent_dim }_vae')
            writer = SummaryWriter(outdir / str(int(time.time())))
            args.save_path_ckpt = SAVE_DIR_CHECKPOINT / f"{args.architecture}_vae"
            args.save_path_img = SAVE_DIR_IMAGES / f"{args.architecture}_{args.latent_dim}_vaeMNIST"
