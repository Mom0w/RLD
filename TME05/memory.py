...
class Memory:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma=0.99, lam=0.95):
        ...
        self.targ_val_buf = np.zeros(size, dtype=np.float32)
        self.logp_buf = np.zeros(size, dtype=np.float32)
        self.gamma, self.lam = gamma, lam
        self.ptr, self.path_start_idx, self.max_size = 0, 0, size

    def store(self, obs, act, rew, val,val_targ, logp,done):
        ...
        self.targ_val_buf[self.ptr] = val_targ
    ...
    def finish_path(self, last_val=0):
        ...
        targ_vals = np.append(self.targ_val_buf[path_slice], last_val)
        
        # the next two lines implement GAE-Lambda advantage calculation
        deltas = rews[:-1] + self.gamma * targ_vals[1:] - vals[:-1]
